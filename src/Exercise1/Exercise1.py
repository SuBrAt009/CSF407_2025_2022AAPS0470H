...
genai.configure(api_key="API KEY")                   #####API KEYS USED< GEMINI AND GROQ
...
def play_game(n, mode="LLM_vs_LLM", human_marker=None):          ####whole logic of playing the game
    ...
    return "Draw"
def run_experiments(n, trials=10):
...
def create_binomial_distribution_chart(results, trials=10):
...
if __name__ == "__main__":
    ...
    if mode_input == "1":
        run_experiments(board_size, trials=10)
        
        results = load_results()
        
        create_binomial_distribution_chart(results, trials=10)
    elif mode_input == "2":
